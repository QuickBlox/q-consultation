...
export default function useForm<T extends object, E extends object>(
  ...
  const setFieldValue = (
    ...
    if (get(values, nameOrPath) === undefined || typeof value === 'object') {
      setTouched({
        ...set<SetTails<T, boolean>>(
          touched,
          nameOrPath,
    ...
    const newValues = { ...set<T>(values, nameOrPath, value) }
  ...
  const setFieldTouched = (nameOrPath: string) => {
    setTouched({ ...set(touched, nameOrPath, true) })
  }
