...
const selector = createMapStateSelector({
  ...
  modalAppointmentChat: modalAppointmentChatSelector,
...
export default createUseComponent((props: AppointmentRegisterProps) => {
  ...
  const { userEntries, usersLoading, avatarEntries } = store
  ...
  const disabledDays = (day: Date | string) => {
    ...
    const dayStart = startOfDay(day instanceof Date ? day : parseISO(day))
    const presentStartDay = startOfDay(presentDay)

    if (type === 'history') return isAfter(dayStart, presentStartDay)

    if (type === 'upcoming') return isBefore(dayStart, presentStartDay)
  ...
  useEffect(() => {
    if (!usersLoading && appointmentList.length) {
      ...
      >(
        (list, { client_id }) =>
          list.includes(client_id) || userIds.includes(client_id?.toString())
            ? list
            : list.concat(client_id),
        [],
      )
  ...
  }, [appointmentList, userEntries, usersLoading])
