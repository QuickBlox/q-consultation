...
import QB, { QBUser, promisifyCall } from '@qc/quickblox'
...
import { authSessionSelector, authMyAccountIdSelector } from '../selectors'
      ...
      if (isSystemMessage(event)) {
        ...
        if (extension?.notification_type) {
          if (
            ...
            yield put(
              getAppointments({
                filters: { _id: extension.appointment_id, limit: 1 },
              }),
            )
          }
